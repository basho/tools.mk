...
${LOCAL_PLT}: compile
	@if [ -d deps ]; then \
		if [ -f $(LOCAL_PLT) ]; then \
			dialyzer --check_plt --plt $(LOCAL_PLT) _build/default/lib/*/ebin  && \
			dialyzer --add_to_plt --plt $(LOCAL_PLT) --output_plt $(LOCAL_PLT) _build/default/lib/*/ebin ; test $$? -ne 1; \
		else \
			dialyzer --build_plt --output_plt $(LOCAL_PLT) _build/default/lib/*/ebin ; test $$? -ne 1; \
		fi \
	fi
...
#   - Output matches for those patterns found in the original warnings
	...
	if [ -f dialyzer.ignore-warnings ]; then \
		...
		dialyzer $(DIALYZER_FLAGS) --plts $${PLTS} -c _build/default/lib/$(PROJECT)/ebin > dialyzer_warnings ; \
		...
		dialyzer $(DIALYZER_FLAGS) --plts $${PLTS} -c _build/default/lib/$(PROJECT)/ebin; \
	fi
